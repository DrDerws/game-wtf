...
- **Unexpected "Indent" / Expected end of file**: These usually show up when a line is indented but the parser is back at the top level. Check the line above for missing dedents or an unclosed multi-line string (for example, `"""` in shader code) that can make the rest of the file look like stray top-level code.
